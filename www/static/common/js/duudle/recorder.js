(function (window) {  
    ...
    var promisifiedOldGUM = function(constraints) {
        ...
        if(!getUserMedia) {
            HZRecorder.throwErr('当前<a href="#" target="_blank" class="keylink">浏览器</a>不支持录音功能。'); 
            return;  
          }
    ...
    //旧的浏览器可能无法实现mediadevices，所以我们先设置一个空的对象
    if(navigator.mediaDevices === undefined) {
      navigator.mediaDevices = {};
    }
    //一些浏览器部分实现mediadevices。我们不能只指定一个对象
    //它将覆盖现有的getUserMedia。
    //在这里，如果它丢失,我们将添加getUserMedia一个。
